...
Require ($defaults.Contains('CONFIG_FREERTOS_USE_TICKLESS_IDLE=y')) 'sdkconfig.defaults must enable Tickless Idle for the staged scheduler-idle step.'
Require ($sdkconfig.Contains('CONFIG_FREERTOS_USE_TICKLESS_IDLE=y')) 'The canonical build sdkconfig must enable Tickless Idle.'
Require ($defaults.Contains('CONFIG_FREERTOS_PLACE_FUNCTIONS_INTO_FLASH=y')) 'Tickless must move eligible non-ISR FreeRTOS functions to flash so Wi-Fi retains enough internal RAM.'
Require ($sdkconfig.Contains('CONFIG_FREERTOS_PLACE_FUNCTIONS_INTO_FLASH=y')) 'The canonical build must preserve the Tickless Wi-Fi internal-RAM fix.'
...
Require ($main -notmatch '[.]light_sleep_enable[ ]*=[ ]*true') 'Automatic Light Sleep must remain disabled for the Tickless-only step.'
Require ($main -match '(?s)#if CONFIG_FREERTOS_USE_TICKLESS_IDLE.*?Tickless Idle enabled.*?#else.*?Tickless Idle disabled.*?#endif') 'The boot log must report the compiled Tickless policy instead of a hard-coded state.'
...
Write-Host 'PASS: 80-240 MHz DFS and Tickless Idle are enabled; non-ISR FreeRTOS functions stay in flash for Wi-Fi RAM headroom; Automatic Light Sleep is disabled.'
