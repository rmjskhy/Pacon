...
$checks = [ordered]@{
    ...
    'Dedicated MSC prepaint must show the requested ON state before TinyUSB starts.' = 's_usb_msc_ui_on\s*=\s*true;[\s\S]{0,180}render_at_max_cpu\(render_usb_disk_frame\);[\s\S]{0,180}start_usb_msc_mode\(\);'
    ...
    'USB Disk dim/sleep sequence must honor the persisted timeout and grace period.' = 'const\s+uint32_t\s+configured_timeout_ms[\s\S]{0,180}uint32_t\s+dim_timeout_ms\s*=\s*configured_timeout_ms;[\s\S]{0,240}configured_timeout_ms\s*\+\s*DISPLAY_SLEEP_AFTER_DIM_MS'
    'USB Disk sleep must keep FT3168 Active for reliable one-touch wake.' = 'if\s*\(s_display_power_state\s*==\s*DISPLAY_POWER_SLEEPING\s*&&\s*s_ui_screen\s*!=\s*UI_SCREEN_USB_DISK\)\s*\{[\s\S]{0,120}touch_enter_monitor\(\);'
    ...
    'USB switch must be raised clear of the round panel edge with a matching normal-mode hit region.' = 'home_in_round_rect\(x,\s*y,\s*170,\s*365,\s*305,\s*417,\s*26\)[\s\S]{0,300}y\s*-\s*391[\s\S]{0,180000}home_in_round_rect\(x,\s*y,\s*155,\s*350,\s*320,\s*432,\s*24\)'
    'USB page must sample the launcher contact release before its slow full-frame render.' = 'else\s+if\s*\(s_ui_screen\s*==\s*UI_SCREEN_USB_DISK\)[\s\S]{0,260}poll_touch\(\);[\s\S]{0,260}if\s*\(s_usb_disk_dirty\)[\s\S]{0,140}render_at_max_cpu\(render_usb_disk_frame\);'
...
if ($firmware -match 'USB_DISK_SLEEP_TIMEOUT_MS') {
    $failures.Add('USB Disk must not override the persisted screen timeout with a fixed constant.')
}
