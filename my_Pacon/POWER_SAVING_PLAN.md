...
- [x] 阶段 6 第一部分代码实现：OuO 根据交互/动画状态使用 60/30 FPS，Fluid 在
  触摸或明显运动时提高帧率、静止时降至约 20 FPS；Fluid 大型仿真工作区放入 PSRAM，
  为 Wi-Fi 与离线中文语音识别保留内部 RAM。
- [x] 阶段 6 第一部分源码回归、ESP-IDF 5.4.3 完整编译和 0u0、Fluid 真机功能验收；
  Fluid 倾斜、触摸和静置降帧期间运动正常，无拖影或明显卡顿。
- [ ] 阶段 6 低电量策略尚未实施；20%/10% 阈值及其亮度、休眠、Wi-Fi 行为需单独实现和验收。
